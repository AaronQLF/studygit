...
- **Canvas** — use the bottom dock or right-click the pane to drop in:
  ...
  - **Page** — Notion-like block editor: press `/` for the slash menu (headings, lists, todos, toggles, callouts, tables, code with syntax highlighting, KaTeX math, Mermaid diagrams, images, dividers, …). Selecting text pops a floating toolbar with formatting, turn-into, link editing, inline AI rewrites (improve / fix grammar / shorter / longer / simplify), and one-click flashcard creation. `Alt+↑/↓` moves blocks, `⌘D` duplicates, the toolbar has a heading outline, and the footer shows live word count + reading time.
  - **Document** — reader where you can highlight any text and thread comments on each highlight
  - **PDF** — uploadable PDF viewer with highlights, threaded comments, AI Q&A over the highlight, and a side-by-side rich notes panel. Virtualized rendering keeps huge textbooks smooth; ⌘F searches inside the document with cross-page match navigation; page jump, anchored zoom, and a dim mode for night reading
  - **Flashcards** — spaced-repetition decks on FSRS (the modern scheduler Anki uses). Three card types: basic Q&A, cloze deletions (`{{hidden words}}`), and image occlusion (drag boxes over a diagram's labels). Generate cards with AI from any source (PDFs, highlights, articles, pages, AI answers) or write them by hand; set an exam date and intervals compress so everything is reviewed in time; grade buttons preview the next interval. The header's **Study** button runs one session over every due card in every workspace and keeps a daily streak
