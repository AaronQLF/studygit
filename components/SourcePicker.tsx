...
// Inline icon renderers. Using a `const Icon = iconFor(kind); <Icon />`
// pattern would trip React 19's `react-hooks/static-components` lint
// because it can't statically prove the function returns a stable
// component reference. Switching on `kind` inside JSX keeps the rule
// happy and the rendered output identical.
function RowIcon({
  kind,
  size,
  className,
...
function NodeRow({
  ...
  // For groups that drill in (>1 citation), show a small chevron on the
  // right so the affordance is obvious. Single-citation groups attach
  // directly — no chevron, just the row itself.
