      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-8 py-7 relative">
        <div className="mx-auto max-w-3xl">
          <div className="mb-3 flex items-center justify-between">
            ...
            {docMode === "annotate" ? (
              <button
                className="text-[11px] text-[var(--pg-muted)] hover:text-[var(--pg-fg)]"
          ...
          {docMode === "write" ? (
            <>
              <input
                className="mb-3 w-full bg-transparent text-3xl font-semibold text-[var(--pg-fg)] outline-none"
                placeholder="Document title"
                value={docTitle}
                onChange={(event) => setDocTitle(event.target.value)}
              />
              <textarea
                className="w-full min-h-[65vh] resize-y rounded-md border border-[var(--pg-border)] bg-[var(--pg-bg-subtle)] px-3 py-3 text-base text-[var(--pg-fg)] outline-none font-serif leading-relaxed"
            ...
            <>
              <h1 className="text-3xl font-semibold text-[var(--pg-fg)] mb-6 leading-tight">
                {documentData.title || "Untitled document"}
              </h1>
              <div
                ref={contentRef}
                className="text-base text-[var(--pg-fg)] leading-relaxed whitespace-pre-wrap font-serif"
                  ...
                  segments.map((segment, idx) => {
                    ...
                    return (
                      ...
                      >
                        {segment.text}
                        {hasComments ? (
                          <sup className="ml-0.5 text-[var(--pg-muted)]">
        ...
        {selection && docMode === "annotate" ? (
          <div
            className="fixed z-[120] -translate-x-1/2 -translate-y-full mt-[-8px] bg-[var(--pg-bg-subtle)] border border-[var(--pg-border-strong)] rounded-lg px-1.5 py-1 flex items-center gap-1 shadow-[var(--pg-shadow-lg)]"
            style={{ top: selection.rect.y, left: selection.rect.x }}
          >
            <Highlighter size={12} className="text-[var(--pg-muted)] ml-0.5" />
            ...
            {activeHighlight ? (
              <div className="flex-1 flex flex-col min-h-0">
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-2">
                  {activeHighlight.comments.length === 0 ? (
                    <div className="px-1 py-2 text-[11px] text-[var(--pg-muted)]">
                      No comments yet.
                    </div>
                  ) : (
                    activeHighlight.comments.map((comment) => (
                      ...
                      >
                        <p className="text-sm text-[var(--pg-fg)] whitespace-pre-wrap">
                          {comment.text}
                        </p>
                        <div className="mt-1 flex justify-between text-[10px] text-[var(--pg-muted)]">
                          <span>
                ...
                <div className="border-t border-[var(--pg-border)] p-2">
                  <textarea
                    className="w-full rounded-md border border-[var(--pg-border-strong)] bg-transparent px-2 py-1.5 text-sm text-[var(--pg-fg)] outline-none resize-none"
                    rows={2}
                    value={commentDraft}
                  ...
                  <div className="mt-1 flex items-center justify-between">
                    <span className="text-[10px] text-[var(--pg-muted)]">⌘↵</span>
                    <button
                      className={clsx(
              ...
              <div className="flex-1 overflow-y-auto p-2 space-y-2">
                {documentData.highlights.length === 0 ? (
                  <div className="px-2 py-3 text-[11px] text-[var(--pg-muted)]">
                    ...
                    .map((highlight) => (
                      ...
                      >
                        ...
                        {highlight.comments.length ? (
                          <div className="mt-1 text-[10px] text-[var(--pg-muted)]">
